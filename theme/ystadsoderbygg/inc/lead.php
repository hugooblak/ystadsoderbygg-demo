 ...
 * or photo. They call ys_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Ystad Söderbygg AB.
 *
 ...
 * @return string
 */
function ys_sample_note() {
	return ys_lead( 'ar_riktig' ) ? '' : ' <span class="ys-sample">(platshållare)</span>';
}
 ...
 * @return string File name for ys_lead_img().
 */
function ys_lead_bild( $i ) {
	$bilder = (array) ys_lead( 'bilder', array() );
 ...
 * @return string
 */
function ys_ring_label() {
	$namn = ys_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, ys_lead( 'telefon' ) ) : 'Ring ' . ys_lead( 'telefon' );
}
