<?php
/**
 * Ystad Söderbygg AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Ystad Söderbygg AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through ys_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'ystadsoderbygg', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'ystadsoderbygg' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'ystadsoderbygg' ) ) );

		register_block_pattern_category( 'ystadsoderbygg-sections', array( 'label' => __( 'Ystad Söderbygg AB sections', 'ystadsoderbygg' ) ) );
		register_block_pattern_category( 'ystadsoderbygg-pages', array( 'label' => __( 'Ystad Söderbygg AB page layouts', 'ystadsoderbygg' ) ) );
	}
);
