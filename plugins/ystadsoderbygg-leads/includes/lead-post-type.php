...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'ystadsoderbygg-leads' ),
					'singular_name' => __( 'Lead', 'ystadsoderbygg-leads' ),
					'edit_item'     => __( 'Förfrågan', 'ystadsoderbygg-leads' ),
					'search_items'  => __( 'Search leads', 'ystadsoderbygg-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'ystadsoderbygg-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'ystadsoderbygg-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'ystadsoderbygg-leads' ),
			'npl_phone'    => __( 'Phone', 'ystadsoderbygg-leads' ),
			'npl_service'  => __( 'Needs', 'ystadsoderbygg-leads' ),
			'npl_timeline' => __( 'When', 'ystadsoderbygg-leads' ),
			'npl_zip'      => __( 'ZIP', 'ystadsoderbygg-leads' ),
			'npl_source'   => __( 'Source', 'ystadsoderbygg-leads' ),
			'npl_status'   => __( 'Status', 'ystadsoderbygg-leads' ),
			'date'         => __( 'Received', 'ystadsoderbygg-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'ystadsoderbygg-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'ystadsoderbygg-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'ystadsoderbygg-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'ystadsoderbygg-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'ystadsoderbygg-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'ystadsoderbygg-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'ystadsoderbygg-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'ystadsoderbygg-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'ystadsoderbygg-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'ystadsoderbygg-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'ystadsoderbygg-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'ystadsoderbygg-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
