...
add_filter(
	...
	function ( $exporters ) {
		$exporters['ystadsoderbygg-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'ystadsoderbygg-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'ystadsoderbygg-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'ystadsoderbygg-leads',
						'group_label' => __( 'Quote requests', 'ystadsoderbygg-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['ystadsoderbygg-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'ystadsoderbygg-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
